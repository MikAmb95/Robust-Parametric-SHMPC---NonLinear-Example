...
for it = 1:NSamps
    ...
    nVar(it) = nu*(MPC.N-it+1);

    % define constraints for Yalmip
    CLax = MPC.qp.A.el{it}*xYAL<=MPC.qp.b;
    % define objective function for Yalmip
    OBJ = .5*xYAL'*MPC.qp.H.el{it}*xYAL + 0.5*MPC.qp.f'*xYAL;
    tic
...
DATA_wo_Param = DATA; stop_wo_P = stop; J_wo_P = J_val; tVec_wo_P = tVec;nVar_wo = nVar;
save('Results/res_wo_P','DATA_wo_Param','stop_wo_P','tVec_wo_P','J_wo_P','nVar_wo');
% warning('off')
% fileName = fullfile(folderDirectory,'results','simulationData');
% save(fileName,'DATA',"extraInfo","commonInfo");
% warning('on')
